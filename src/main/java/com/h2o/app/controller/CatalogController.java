...
public class CatalogController {
    ...
    public List<Tanker> tankers(@RequestParam String city,
        ...
        return tankerRepository.findByCityIgnoreCaseAndUsageTypeIgnoreCaseAndAvailableTrue(city, usageType)
                .stream()
                .filter(t -> (t.getArea() != null && t.getArea().toLowerCase().contains(key)) ||
    ...
    public List<Product> products(@RequestParam String city,
        ...
        String key = location.trim().toLowerCase();
        return productRepository.findByCityIgnoreCaseAndServiceTypeIgnoreCaseAndAvailableTrue(city, serviceType)
                .stream()
                .filter(p -> (p.getArea() != null && p.getArea().toLowerCase().contains(key)) ||
